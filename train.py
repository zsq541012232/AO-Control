...
from data_utils import split_dataset, get_indices_from_dir, ZernikeDataset, ZernikeDatasetFixed3Channel, split_train_val
...
from model import (ConsistentUnderCorrectLoss,
                   SignWeightedResidualLaplaceLogLikelihoodLoss,
                   SignWeightedResidualStudentTLogLikelihoodLoss,
                   ConsistentUnderCorrectResidualStudentTLogLikelihoodLoss,
                   ConsistentUnderCorrectResidualLaplaceLogLikelihoodLoss)
import pandas as pd
...
def train():
    ...
    use_independent_test_dir = False
    
    train_dir = "dataset/ao_simulated-simple"
    # train_dir = "../dataset/train/imgData-r6-35"
    ...
    if use_independent_test_dir:
        train_idx, val_idx = split_train_val(train_dir, val_size=0.1)
    else:
    ...
    train_dataset = DatasetClass(data_dir=data_dir_train,
                                 indices=train_idx,
                                 prefixes=prefixes,
                                 num_modes=num_modes,
                                 use_log_preprocess=False)
    val_dataset = DatasetClass(data_dir=data_dir_val,
                               indices=val_idx,
                               prefixes=prefixes,
                               num_modes=num_modes,
                               use_log_preprocess=False)
    ...
    if use_fixed_3channel:
        ...
        # model = ZernikeMambaPure(num_outputs=num_modes, in_channels=model_in_channels, img_size=128).to(device)
        # model = ZernikeFusionMambaPure(num_outputs=num_modes, img_size=224).to(device)
        # model = ZernikeUNetMambaDeepFusion(num_outputs=num_modes, in_channels=model_in_channels, img_size=128).to(device)
    ...
    # criterion = ConsistentUnderCorrectLoss(mse_weight=1.0,margin=0.0,sign_penalty=0.8,over_weight=0.3).to(device)
    # print(f"    ✅ 已使用 ConsistentUnderCorrectLoss（margin=0.0, sign_penalty=8.0, over_weight=3.5）")
    # criterion = SignWeightedResidualLaplaceLogLikelihoodLoss(penalty_weight=sign_penalty)
    # print (f"    ✅ 已使用 SignWeightedResidualLaplaceLogLikelihoodLoss")
    # criterion = SignWeightedResidualStudentTLogLikelihoodLoss(penalty_weight=sign_penalty)
    # print (f"    ✅ 已使用 SignWeightedResidualStudentTLogLikelihoodLoss")
    # criterion = ConsistentUnderCorrectResidualLaplaceLogLikelihoodLoss(sign_penalty=sign_penalty, over_weight=over_weight)
    # print (f"    ✅ 已使用 ConsistentUnderCorrectResidualLaplaceLogLikelihoodLoss")
    # criterion = ConsistentUnderCorrectResidualStudentTLogLikelihoodLoss(sign_penalty=sign_penalty, over_weight=over_weight)
    # print (f"    ✅ 已使用 ConsistentUnderCorrectResidualStudentTLogLikelihoodLoss")
